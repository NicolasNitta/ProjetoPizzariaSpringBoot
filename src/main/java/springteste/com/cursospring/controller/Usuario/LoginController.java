...
public class LoginController {
    ...
    public ResponseEntity validacaoCredenciais(@RequestBody @Valid CredenciaisDTO dto, UriComponentsBuilder builder) {
        UsernamePasswordAuthenticationToken token = new UsernamePasswordAuthenticationToken(dto.getLogin(),
                dto.getPassword()); // como se fosse outro dto com os atributos sendo passado pelos parametros

        Authentication autenticacao = autenticador.authenticate(token);// ver se existe no banco de dados
        return ResponseEntity.ok(tokenService.criarToken((Usuario) autenticacao.getPrincipal()));// criação do token
                                                                                                 // para o login
    }
